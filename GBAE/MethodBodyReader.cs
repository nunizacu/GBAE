...
namespace SDILReader
{
    public class MethodBodyReader
    {
            ...
            while (position < il.Length)
            {
                ...
                switch (code.OperandType)
                {
                    ...
                    case OperandType.InlineField:
                        metadataToken = ReadInt32(il, ref position);
                        instruction.Operand = resolver.ResolveField(metadataToken, null, null);
                        break;
                    case OperandType.InlineMethod:
                        metadataToken = ReadInt32(il, ref position);
                        try
                        {
                            instruction.Operand = resolver.ResolveMethod(metadataToken,null,null);
                        }
                        catch
                        {
                            instruction.Operand = resolver.ResolveMember(metadataToken,null,null);
                        }
                        break;
                    case OperandType.InlineSig:
                        metadataToken = ReadInt32(il, ref position);
                        instruction.Operand = resolver.ResolveSignature(metadataToken);
                        break;
                    case OperandType.InlineTok:
                        metadataToken = ReadInt32(il, ref position);
                        try
                        {
                            instruction.Operand = resolver.ResolveType(metadataToken,null,null);
                        }
                        catch
                        {
                    ...
                    case OperandType.InlineType:
                        ...
                        instruction.Operand = resolver.ResolveType(metadataToken, this.mi.DeclaringType.GetGenericArguments(), this.mi.GetGenericArguments());
                        break;
                    ...
                    case OperandType.InlineString:
                        {
                            metadataToken = ReadInt32(il, ref position);
                            instruction.Operand = resolver.ResolveString(metadataToken);
                            break;
                        }
        ...
        public MethodBodyReader(DynamicMethod dyn)
        {
            this.mi = mi;
            this.mo = mo;
            this.il = dyn.GetILAsByteArray();
            this.resolver = new DynamicMethodTokenResolver(dyn);
